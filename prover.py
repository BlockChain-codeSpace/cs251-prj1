...
def gen_merkle_proof(leaves, pos):
    ...
    for level in range(height):
        level_leaves = 2**(height - level - 1)
        level_array = []
        for i in range(level_leaves):
            if (level == 0) :
                level_array += [hash_internal_node(state[i*2], state[i*2 + 1])] 
            else : 
                level_array += [hash_internal_node(full_leaves[level - 1][i*2],full_leaves[level - 1][i*2 + 1])]
        full_leaves += [level_array]
    if (level_pos) % 2 == 0:
        path += [state[level_pos + 1]]
    else:
        path += [state[level_pos - 1]]
    for level in range(height):
        level_pos //= 2
        if (level == height - 1):
            ...
            path += [full_leaves[level][level_pos + 1]]
        else:
            path += [full_leaves[level][level_pos - 1]]
